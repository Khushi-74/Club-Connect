...
public class postImage extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        progressDialog.setMessage("Keep Hustling...");
...
// now we need a model class
            UserActivity.ItemModel model = new UserActivity.ItemModel(Name, Description, "", UrlsList);
            firestore.collection("Events").add(model).addOnSuccessListener(new OnSuccessListener<DocumentReference>() {
                @Override
                public void onSuccess(DocumentReference documentReference) {
// now here we need Item id and set into model
                    model.setItemId(documentReference.getId());
                    firestore.collection("Events").document(model.getItemId())
...
// if data uploaded successfully then show ntoast
                                    Toast.makeText(postImage.this, "Your Event posted Successfully", Toast.LENGTH_SHORT).show();
...
// if you want to clear viewpager after uploading Images
    ...
    private void CheckPermission() {
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.M) {
            if (ContextCompat.checkSelfPermission(postImage.this,
                    Manifest.permission.READ_EXTERNAL_STORAGE) != PackageManager.PERMISSION_GRANTED) {
                ActivityCompat.requestPermissions(postImage.this, new
